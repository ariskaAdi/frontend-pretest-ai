...
import { useModuleDetailQuery, useDeleteModuleMutation, useRetrySummarizeMutation } from '@/queries/useModuleQuery'
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-40">
        ...
        <p className="text-gray-500 font-medium animate-pulse">Memuat detail modul...</p>
      </div>
    )
  }
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap items-center gap-4 text-sm">
              ...
              >
                {module.is_summarized ? 'Ringkasan Tersedia' : module.summarize_failed ? 'Gagal Merangkum' : 'Sedang Diproses AI'}
              </Badge>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <Card className="p-6 border-none shadow-sm rounded-3xl bg-white flex flex-col items-center text-center group cursor-pointer hover:shadow-md transition-all duration-300">
          ...
          <a href={module.file_url} target="_blank" rel="noopener noreferrer" className="w-full">
            <Button variant="ghost" className="w-full rounded-xl">Buka Link</Button>
          </a>
        </Card>
        ...
        <Card className="p-6 border-none shadow-sm rounded-3xl bg-white flex flex-col items-center text-center group cursor-pointer hover:shadow-md transition-all duration-300">
          ...
          <p className="text-xs text-gray-400 mb-4">
            {module.summarize_failed ? 'Terjadi kesalahan saat merangkum materi.' : 'Lihat rangkuman cerdas dari AI'}
          </p>
